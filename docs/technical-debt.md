...
- 🔄 **ĐANG LÀM — thiết kế lại UI báo cáo lỗi, nhánh `feat/ui-bao-cao-loi-ai` (CHƯA MERGE).** Phạm vi cố ý hẹp: **chỉ PHP/JS/CSS trong `drupal/web/modules/custom/vf_ai_review/`, không sửa một dòng Python nào** — đã xác minh diff đường chấm điểm vẫn rỗng nên E1/E5/E6 còn hiệu lực. Phần **hiển thị đã xong**, kiểm bằng ảnh chụp trình duyệt thật qua 4 vòng sửa. Phần **tương tác (Trước/Sau, lọc, localStorage) CHƯA AI KIỂM** — dự án không có hạ tầng test JS, đừng ghi là đã verify. Thiết kế, ba thứ cố ý cắt, và **bốn cái bẫy Drupal đã gặp**: [`editor-ui-design.md` mục 10](editor-ui-design.md). Test: `cd drupal && ddev exec php scripts/test_ai_report_renderer.php` (75 PASS).
