...
Cả 4 agent có chung một cơ chế vận hành: nhận vào các field nội dung liên quan (`src/prompt_builder.py` ghép chúng thành một khối bọc trong thẻ có hậu tố ngẫu nhiên - xem cảnh báo an ninh dưới đây), gửi cho LLM (Claude) kèm theo một system prompt (bộ chỉ dẫn/tiêu chí) cố định và riêng biệt cho từng agent, yêu cầu trả về kết quả theo một cấu trúc JSON cố định (structured output). **Mỗi lỗi/flag trong output đều gắn trường `field`** cho biết nó thuộc field nào — nhờ đó Write-back Node gom được báo cáo theo từng field (mục 2.3). "4 agent" về bản chất là cùng một mô hình LLM được gọi 4 lần với 4 bộ chỉ dẫn khác nhau, không phải 4 mô hình AI riêng biệt.

Các field mỗi agent đọc: Content Quality đọc `title/body/summary`; SEO đọc `title/meta_description/url_alias/body/image_alt`; Brand Voice đọc `title/body/summary`; Compliance đọc `title/body/meta_description`.

**Cảnh báo an ninh về cách ghép nội dung - đã xử lý (2026-08-04):** cách ghép cũ dùng nhãn text thuần (`[title]`, `[body]`...) nên **người viết giả mạo được** - gõ đúng chuỗi đó vào body là xoá được ranh giới giữa dữ liệu và chỉ dẫn. Nguy hiểm hơn: field `body` của Drupal là HTML, nên chỉ dẫn giấu trong bình luận HTML hoặc phần tử `display:none` **vô hình với người duyệt nhưng hiển thị với LLM**. Nay `src/prompt_builder.py` bọc nội dung trong thẻ có hậu tố ngẫu nhiên (M1) và bóc phần ẩn trước khi đưa vào prompt (M3); Compliance thêm CP9 coi chỉ dẫn ẩn là vi phạm tuân thủ. Phân tích đầy đủ: `docs/prompt-injection.md`.

**Cách chấm điểm: xem `docs/rubrics.md`.** Ở Sprint 1-2, mỗi agent trả thẳng `score: 0-100` do LLM tự đặt - nhưng không chỗ nào định nghĩa 85 khác 70 ở điểm gì, nên điểm không tái lập được và không calibrate được (chính lập luận đã dùng để bắt Aggregator phải tất định ở mục 6 cũng áp dụng cho từng agent). `rubrics.md` thay bằng: LLM (và một phần là code, với các tiêu chí đếm được) chỉ **phân loại mức từng tiêu chí** kèm trích dẫn nguyên văn làm bằng chứng, rồi một hàm tất định quy các mức đó ra điểm. Riêng Compliance, cả `score` lẫn `severity` đều chuyển sang tất định - severity tra bảng theo mã tiêu chí, không để LLM tự chọn, vì `critical` là thứ kích hoạt quyền phủ quyết. Rubric v1 **đã vào code cho Brand Voice và Compliance** (2026-08-04); `content_quality` và `seo` vẫn để LLM tự cho `score` - E1 đo được σ = 0,13 và 0,30 nên chưa chặn gì (`rubrics.md` mục 8.1, `technical-debt.md` A1). *(Hai số này đo trên bộ `e1_stability_rubric_v2.json`; bảng ở `technical-debt.md` A1 ghi 0,38 và 0,19 vì đo trên bộ `e1_stability_raw.json` — hai lần chạy khác nhau, không mâu thuẫn. Trích σ nào cũng phải kèm tên bộ dữ liệu.)* Các mục 5.1-5.2 dưới đây mô tả output của hai agent chưa chuyển.
...
**Trục 2 - ngôn ngữ.** Ba điểm giữ sẵn để mở rộng mà không đập đi làm lại: (1) `langcode` là tham số đầu vào của Orchestrator và mọi agent, không hard-code "vi" — ⚠️ **điểm này hiện CHƯA đúng hoàn toàn trong code**: `graph.py` không truyền `content_type`/`langcode` từ state xuống `brand_voice.run()` và `compliance.run()`, nên hai truy vấn RAG luôn lọc theo hằng số mặc định. Chưa gây sai kết quả vì mới có một khoá thật, nhưng phải sửa trước khi thêm khoá thứ hai (`docs/technical-debt.md` B6); (2) cùng cơ chế config `(content_type, langcode)` ở trục 1 đã bao luôn trục này; (3) lớp phân tích ngôn ngữ (tách từ, đếm câu, readability - mục 5.5) tách sau một interface chung, hiện chỉ implement cho tiếng Việt, thêm ngôn ngữ mới = thêm một class, không đụng 4 agent.
...
**Vì sao không lấy thẳng kết quả AHP (0.42/0.23/0.23/0.12) làm trọng số chính thức luôn?** Vì AHP **không tạo ra tính khách quan** - nó chỉ giúp một bộ phán đoán chủ quan trở nên *nhất quán và truy vết được*. Toàn bộ kết quả 0.42/0.23/0.23/0.12 bị quyết định bởi các phán đoán so sánh cặp (2/2/3/1/2/2) mà chính người thiết kế tự đặt để minh họa, **chưa** phải do chuyên gia VF O2O đánh giá - đầu vào là phỏng đoán thì đầu ra cũng vẫn là phỏng đoán, chỉ trông "kỹ thuật" hơn. Ngoài ra, đẩy Compliance lên 0.42 sẽ **đếm tầm quan trọng của nó hai lần** (đã có quyền phủ quyết ở mục 6.2, lại thêm trọng số rất cao), làm méo điểm của các bài không có lỗi critical. Do đó AHP ở đây chỉ đóng vai trò **minh họa phương pháp suy trọng số có kỷ luật**; muốn dùng AHP thật thì cần chuyên gia VF O2O trực tiếp so sánh cặp, còn con số cuối cùng vẫn phải qua calibration gold set ở Sprint 3 (mục 8.2) mới đủ căn cứ.
